...
public class TestClass {
	...
	public static void main(String args[]) throws Exception {
		...
		for (int i = 1; i <= Integer.parseInt(nofCases); i++) {
			...
			while(true){
				...
				if(null == dataInputLine | dataInputLine.equalsIgnoreCase("")){
					casesandInputs.put(new Integer(i), inputs);
					executeAllCases(nofCases,casesandInputs);
					
				}
				if(!isNumeric(dataInputLine.split("\\s")[0])){
		...
		/*for (int i = 1; i <= Integer.parseInt(nofCases); i++) {
			...
			System.out.println(calculateandPrint(casesandInputs.get(i).toArray(
					new String[size])));
		}*/
	...
	private static String calculateandPrint(String[] input) {
		...
		for (int i = 1; i <= input.length - 1; i++) {
			...
			System.out.println("inpt split:" + passDetails);
			...
			if (kindOfPass.equalsIgnoreCase("P")) {
				...
				System.out.println("Previuos Player:" + previosPlayer);
				System.out.println("Current  Player:" + currentPlayer);
				tmpForBack = String.copyValueOf(currentPlayer.toCharArray());
				previosPlayer = currentPlayer;
			...
			if (currentPass == noOfPassesInput) {
				System.out.println("Current  pass = noOfPassesRequired:"
						+ noOfPassesInput);
				return currentPlayer;
			}
		}
